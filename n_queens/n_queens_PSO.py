...
def conflicts(state):
    N = len(state)
    return sum(
        conflict(state[c1], c1, state[c2], c2)
        for c1 in range(N)
...
def pso(n_sub, n_parts, n_iter):
    ...
    particles = [
        {
            "states": [rand.randint(0, n_parts - 1) for _ in range(n_sub)],
            "pbest": [-1] * n_sub,
            "pbest_score": 0,
            "v": [rand.randint(0, r_maxv) for _ in range(n_sub)],
        }
        ...
        for particle in particles:
            ...
            for particle_idx, particle in enumerate(particles):
                for state_idx, state_val in enumerate(particle["states"]):
                    r = rand.randint(0, 2)
                    particle["states"][state_idx] = (
                        inertia * particle["v"][state_idx]
                        + cognitive * r * (particle["pbest"][state_idx] - state_val)
                        + social * r * (gbest[state_idx] - state_val)
                    )
                    particle["states"][state_idx] = (
                        particle["states"][state_idx] + particle["v"][state_idx]
                    ) % n_sub
